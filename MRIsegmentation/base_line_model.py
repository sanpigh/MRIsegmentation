...
class BaseLineModel:
    def __init__(
        self, red_threshold_coef=1.5, green_threshold_coef=1.7, blue_threshold_coef=1.5
    ):
    ...
    def predict(self, X):
        ...
        for index, img in enumerate(X):
            ...
            red_threshold = tf.math.reduce_mean(
                img[:, :, 0]
            ).numpy() + self.red_threshold_coef * tf.math.reduce_std(
                img[:, :, 0].numpy()
            )
            green_threshold = tf.math.reduce_mean(
                img[:, :, 1]
            ).numpy() + self.green_threshold_coef * tf.math.reduce_std(
                img[:, :, 1].numpy()
            )
            blue_threshold = tf.math.reduce_mean(
                img[:, :, 2]
            ).numpy() + self.blue_threshold_coef * tf.math.reduce_std(
                img[:, :, 2].numpy()
            )
